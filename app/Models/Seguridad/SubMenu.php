...
class SubMenu
{
    ...
    {
        $stmt = app('con_ibs')->prepare('SELECT ' . implode(', ', self::getFields()) . ' FROM BADCYFILES.SRLSUBMENU WHERE SUB_CODMEN = :menu ORDER BY DESCRIPCION');
        $stmt->execute([
            ':menu' => $menu,
        ]);
    ...
    {
        $stmt = app('con_ibs')->prepare('SELECT ' . implode(', ', self::getFields()) . ' FROM BADCYFILES.SRLSUBMENU WHERE SUB_CODMEN = :menu AND SUB_CODIGO = :codigo');
        $stmt->execute([
            ':menu' => $menu,
            ':codigo' => $codigo,
        ]);
    ...
    public static function getByMenuAccess($menu, $usuario) {
        $stmt = app('con_ibs')->prepare('SELECT ' . implode(', ', self::getFields()) . ' FROM BADCYFILES.SRLSUBMENU S, BADCYFILES.SRLACCESOS A WHERE S.SUB_CODMEN = :menu AND A.ACC_ESTADO = \'A\' AND S.SUB_ESTATU = \'A\' AND S.SUB_WEB = \'S\' AND S.SUB_CODMEN = A.ACC_CODMEN AND S.SUB_CODIGO = A.ACC_SUBMEN AND A.ACC_USER = :usuario');
        $stmt->execute([
            ':menu' => $menu,
            ':usuario' => $usuario,
        ]);
    ...
    {
        $stmt = app('con_ibs')->prepare('SELECT SUB_CODIGO CODIGO FROM BADCYFILES.SRLSUBMENU WHERE SUB_CODMEN = :menu ORDER BY SUB_CODIGO DESC FETCH FIRST 1 ROWS ONLY');
        $stmt->execute([
            ':menu' => $menu
        ]);
        $result = $stmt->fetch();
